...
7. THE Overlay Page SHALL send log entries to the backend via the `/overlay` WebSocket namespace (not the REST logs API, which requires authentication). The backend SHALL write these entries to the unified logger with `source: "overlay"`, producing the `[overlay]` prefix in log output (distinct from `[backend]` and `[frontend]`). This ensures that catastrophic overlay errors are captured in the unified log file for post-incident debugging.
