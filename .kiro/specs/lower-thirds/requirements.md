...
7. THE Overlay Page SHALL be able to send log entries to the backend for debugging. THE backend SHALL expose the `POST /api/logs` endpoint without authentication (or the overlay SHALL send logs via the `/overlay` WebSocket namespace). Log entries from the overlay SHALL be tagged with `source: "overlay"` in the backend logger. This ensures that catastrophic overlay errors are captured in the unified log file for post-incident debugging.
