...
5. THE backend SHALL cache the page breakdown for each scripture library item. When the item is later promoted to active, the overlay receives the pre-computed pages and does not re-measure. The dashboard displays the page count in the item's subtitle (e.g., "Scripture · 3 pages") immediately after measurement completes.
6. IF the overlay is not connected when a scripture item is added, measurement SHALL be deferred until the overlay connects. The library item SHALL display "Scripture · Pending" in its subtitle until measurement completes. The item MAY still be staged and promoted to active before measurement — in that case, measurement occurs at activation time (worst case fallback).
7. THE volunteer SHALL control page navigation via Previous/Next buttons in the widget. Page commands flow: dashboard → backend → overlay. The overlay animates the page transition and reports the new phase.
8. FOR the width optimization (Req 6.9 — expanding from 70% to 80% if it eliminates a wrap): THE Overlay Page SHALL perform this measurement during the pagination calculation, testing both widths and choosing the narrower width unless the wider width reduces the total line count.
...
2. THE backend SHALL emit the following to the `/overlay` namespace: (a) `show` — display a lower-third (includes full item data, style, and content), (b) `dismiss` — trigger the dismiss animation, (c) `push-up` — trigger a direct content swap (includes new item data), (d) `page` — navigate to a specific scripture page, (e) `state` — full state sync (sent on connection for initial state), (f) `measure` — request scripture pagination measurement (includes verse data; overlay measures in hidden container without displaying).
