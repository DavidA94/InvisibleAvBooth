...
**OAuth localhost constraint**: The OAuth redirect URIs are registered as `https://localhost/api/auth/callback/{platform}` in the Google Cloud Console and Facebook Developer portal. Google and Facebook reject non-public domains like `invisible.av`, so `localhost` is used for the callback. This means the OAuth consent flow must be initiated from a browser on the machine running the backend — the redirect lands on `localhost`, which Caddy routes to the backend. This is a one-time admin setup task. After tokens are stored, all other operations (admin config, volunteer streaming) work from any device on the LAN via `invisible.av`. If a public domain is available (e.g., `av.yourchurch.org`), it can be used instead of `localhost` by updating the redirect URIs in the respective developer consoles and setting `APP_URL` in `.env`.
