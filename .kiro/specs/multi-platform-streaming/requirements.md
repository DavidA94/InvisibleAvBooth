...
5. EACH platform admin page SHALL include a "Connect" button that initiates the OAuth 2.0 authorization flow for that platform, redirecting the admin to the platform's consent screen and handling the callback to store the resulting tokens. OAuth setup MUST be performed from a browser on the machine running the backend, because the OAuth callback URL uses `localhost` (Google and Facebook reject non-public domains like `invisible.av`). If a public domain is configured, OAuth can be performed from any device. This is a one-time admin task documented in `docs/setup.md`.
...
7. THE Backend SHALL validate stored OAuth tokens on startup by making a lightweight API call to each enabled platform; IF a token is invalid or expired and cannot be refreshed, THE Backend SHALL emit a Banner-level notification: "{Platform} authorization expired — an admin needs to reconnect." FOR Facebook, THE Backend SHALL additionally validate that the stored Page ID is still accessible; IF the Page is inaccessible, THE Backend SHALL emit a Banner-level notification: "Facebook Page '{Page Name}' is no longer accessible — an admin needs to reconnect."
...
10. THE ManageStreamsModal SHALL display the YouTube privacy setting alongside the platform name (e.g., "YouTube (Unlisted)"). FOR AvVolunteer users, this is display-only. FOR ADMIN and AvPowerUser users, THE ManageStreamsModal SHALL display a privacy dropdown on the YouTube row allowing override to `public`, `unlisted`, or `private` — defaulting to the admin-configured value. The override applies only to the current broadcast and does not persist to the admin configuration. FOR Facebook, THE ManageStreamsModal SHALL display a greyed-out privacy indicator with a popover (triggered on the wrapping container element, since Ionic popovers do not fire on disabled elements) explaining: "Facebook does not support privacy settings for Page live videos."
