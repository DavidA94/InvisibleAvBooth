...
8. THE template create/edit modal SHALL use a two-step validate-then-save flow: (a) the modal initially shows a "Validate" button (not "Save"), (b) tapping "Validate" runs validation checks, (c) if no blocking errors exist the button changes to "Save", (d) any edit to the Name or Format String fields (input event with actual data change) reverts the button back to "Validate". Role Minimum changes do NOT revert the button.
9. VALIDATION SHALL check: (a) unknown placeholder tokens in the format string — BLOCKER; (b) duplicate format string (case-sensitive, whitespace-collapsed comparison against all other templates in the same category, excluding self when editing) — BLOCKER, message: "Duplicate of {existingTemplateName}"; (c) non-unique name within the same category — BLOCKER; (d) if `roleMinimum` is `AvVolunteer` and another template in the same category already has `roleMinimum: AvVolunteer` — WARNING (amber), message: "Volunteers will see multiple {title/description} templates and must choose between them." Blockers are shown as red errors; warnings are shown as amber. The "Save" button SHALL NOT appear while any blocker exists.
...
10. WHEN OBS disconnects from the relay while FFmpeg forwarder processes are active, THE Backend SHALL immediately transition all active platform statuses to "No Source" (distinct from "Streaming", "Error", or "Idle"). THE ManageStreamsModal SHALL display "No Source — waiting for OBS to reconnect…" for each affected platform with no action button. WHEN OBS reconnects to the relay and data resumes, THE Backend SHALL transition each platform from "No Source" to "Recovering" and poll the platform's API to verify the broadcast is still active. IF the API confirms the broadcast is active, THE Backend SHALL transition the platform to "Streaming" and auto-clear any related Banner. IF the API reports the broadcast has ended (e.g., YouTube's `enableAutoStop` fired during the gap), THE Backend SHALL transition the platform to "Error" with message "{Platform} broadcast ended during disconnect — tap Start to create a new stream."
11. THE OBS Widget's `WidgetContainer` SHALL display a "Relay" connection status indicator alongside the existing "OBS" indicator. The relay indicator SHALL use a four-state model: (a) green solid dot (`healthy`) — relay is running and OBS is connected to it, (b) grey solid dot (`inactive`) — relay is running but OBS is not connected to it (normal pre-stream state), (c) red blinking dot (`unhealthy`) — relay has failed to start or has crashed and recovery failed, (d) grey solid dot (`inactive`) — no streaming platforms are configured or enabled (relay is running but unused). The grey dot SHALL use `color-text-muted` (`#A0A0A0`) which has a verified ~5.3:1 contrast ratio against `color-bg`. WHEN the relay is in the `unhealthy` state, THE "Manage Streams" button SHALL be disabled with sub-label "Streaming unavailable".
...
### Risk 1: 30–60 Second Stream Start Sequence Under API Latency

The multi-step platform start sequence (create broadcast → get RTMP URL → ensure OBS streaming → spawn FFmpeg) may take up to 30 seconds per platform. With two platforms, worst case is 60 seconds. YouTube API latency spikes are common on Sunday mornings. The step-level progress indicator (Req 6.3) and widget-level "Starting…" feedback (Req 6.11) provide visibility but do not reduce the wait. This is accepted — pre-creating broadcasts would add complexity and stale-broadcast cleanup logic that is not justified for the initial release.
