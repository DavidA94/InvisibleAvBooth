...
3. WHEN a user authenticates and has no cached dashboard ID in localStorage, THE Frontend SHALL display the Dashboard Selection Screen showing all accessible dashboards with their name and description. *(Modified by multi-platform-streaming Req 11.3: ADMIN users are redirected to `/admin` instead of the Dashboard Selection Screen.)*
4. WHEN a user authenticates and has access to exactly one dashboard and no cached dashboard ID, THE Frontend SHALL automatically select that dashboard without showing the selection screen. This auto-select behavior applies only on initial authentication — tapping the dashboard navigation label in the `GlobalTitleBar` always navigates to the Dashboard Selection Screen regardless of how many dashboards are accessible. *(Modified by multi-platform-streaming Req 11.3: ADMIN users are redirected to `/admin` instead of the Dashboard Selection Screen after login.)*
...
12. THE 'Start Stream' button SHALL be disabled unless the SessionManifest contains at least one of `speaker` or `title` *(Superseded by multi-platform-streaming Req 12.4: replaced by `manifestReady` boolean and priority-ordered sub-label system on the "Manage Streams" button)*; THE OBS Widget SHALL display a sub-label "Enter metadata" on the disabled button; WHEN the user taps the disabled button and the only reason it is disabled is missing metadata (OBS is connected, no other blocker), THE OBS Widget SHALL open the SessionManifestModal directly; WHEN the user taps the disabled button and OBS is disconnected or another blocker is active, THE OBS Widget SHALL display a Toast notification explaining the reason (e.g., "OBS is not connected").
...
1. THE Backend SHALL maintain a configurable metadata template string (e.g., `{Date} – {Speaker} – {Title}`) for OBS stream metadata; IF no template is configured for a device connection, THE Backend SHALL use the default `"{Date} – {Speaker} – {Title}"`. *(Superseded by multi-platform-streaming Req 3: replaced by the `metadata_templates` table with named templates, categories, and role-based visibility.)*
