# Requirements Document — Lower Thirds with Preview
...
7. IF the detected resolution is incorrect, THE Overlay Page SHALL render a high-visibility diagnostic box (bright border, warning text) that is visible only during the initial connection phase (before any lower-third has been activated). This diagnostic box SHALL be hidden once the first lower-third is shown or after 30 seconds, whichever comes first.
...
6. LOWER-THIRD templates SHALL be de-duplicated by their JSON `formatString` content (normalized: keys sorted, whitespace collapsed) within the `'lower_third'` category. Duplicate detection SHALL compare the full JSON blob, not individual fields.
...
6. THE service SHALL own auto-dismiss timers. WHEN an item with `autoDismissMs` is promoted to Active, THE service SHALL start a timer. WHEN the timer fires, THE service SHALL send a dismiss command to the overlay page and emit a `dismissing` phase event to dashboard clients.
...
13. ADDING a new item to the library SHALL NOT shift the scroll position of the Library section. New items appear at the bottom (newest last).
...
11. FOR Scripture multi-verse (small — fits in 4 lines or fewer): each verse starts a new line with a number prefix (e.g., "1. ..."). Verse 0 has no number prefix and is displayed in italics. The line width cap SHALL be calculated to favor the longest verse, expanding only if it eliminates at least one line from the total height.
12. FOR Scripture multi-verse (large — exceeds 4 lines): the content SHALL be paginated. The scripture reference title line remains fixed across all pages. No verse SHALL be split across pages. The volunteer controls pagination via the dashboard widget (Req 5.4).
...
3. THE Overlay Page SHALL determine page breaks such that: (a) no single verse is split across pages, (b) each page fits within 4 lines of rendered text (at the calculated width), (c) verse 0 (if present) is always on the first page.
...
7. WHEN the overlay disconnects from the `/overlay` namespace, THE backend SHALL NOT immediately clear the active state. The overlay may reconnect and resume display. IF the overlay has been disconnected for more than 30 seconds and there is an active lower-third, THE backend SHALL mark the active item as stale (for dashboard display purposes) but SHALL NOT send dismiss commands to a disconnected overlay.
8. WHEN the overlay reconnects, THE backend SHALL send the full current state. IF an item was active when the overlay disconnected, the overlay SHALL re-render it immediately (no entrance animation) and report phase `visible`.
...
4. WHEN a volunteer edits a staged or library item, THE edit dialog SHALL pre-populate with the item's current values. Saving an edit to a staged item SHALL update it in place without changing its staged status.
...
10. THE widget SHALL display a brief inline message "No items available" in the Library section when the library is empty (no templates resolvable and no volunteer-added items).
