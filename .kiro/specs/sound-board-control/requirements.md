...
3. WHEN the user toggles the mute button, THE Frontend SHALL send the corresponding mute command to the backend for that channel, and THE Backend SHALL forward it to the mixer (correctly mapping to the inverted `mix/on` sense). **The mute control SHALL NOT optimistically flip to the commanded state.** Immediately on toggle it SHALL enter the **unconfirmed** state ("Audio: Unknown", yellow dot — Req 6.6) and remain there until the mixer's own value arrives (read-back success or `/xremote` push), then resolve to the mixer-reported On/Off. This prevents a brief false "Audio: On/Off" during the send→confirm gap (~1 read-back round-trip); because a false "Audio: Off" on a live mic is the worst-case audio error, mute never displays a value the mixer has not confirmed.
...
6. THE mute control SHALL show an explicit **unconfirmed** state — a **yellow dot with the text "Audio: Unknown"** — whenever the true mute state is not confirmed by the mixer: (a) from the moment of a local toggle until the mixer's value arrives (Req 6.3), and (b) if a mute command's read-back cannot be confirmed after bounded retry (Req 2.7). It SHALL NOT display "Audio: On" or "Audio: Off" for an unconfirmed mute (a false "Off" would be a silent open mic; a false "On" would be a false alarm). The state SHALL resolve to the mixer-reported On/Off as soon as the mixer confirms the value (via read-back success or an `/xremote` push). This is the mute case of the general unreconciled-state handling (Req 8/Req 15).
