...
| **`/headamp/NNN/gain` (float, −12.0…+60.0 dB)**                      | Preamp gain                                                   | The XR18's MIDAS preamps provide a **−12 dB to +60 dB** range (72 dB). Headamp index is a separate zero-padded space (`/headamp/000`). Source: `xair-api-python` `HeadAmp.gain` documents "float, from -12.0 to 60.0"; corroborated by the XR18 product specification (MIDAS preamps).                                                                                                                                                                                                                                                                                                                                                     |
...
2. THE number of channels exposed SHALL be determined by the admin-configured **channel count** for the device (Req 9), not by the model name.
...
3. WHEN the user toggles the mute button, THE Frontend SHALL send the corresponding mute command to the backend for that channel, and THE Backend SHALL forward it to the mixer (correctly mapping to the inverted `mix/on` sense).
4. WHEN the backend reports a mute-state change for a channel (from any origin, including external changes), THE widget SHALL update the button appearance and the "Audio: On/Off" text and dot accordingly.
...
6. IF a mute command's read-back cannot be confirmed after bounded retry (Req 2.7), THE widget SHALL show an explicit **unconfirmed** state — a **yellow dot with the text "Audio: Unknown"** — rather than assuming either "On" or "Off". It SHALL NOT display "Audio: Off" for an unconfirmed mute (a false "Off" would be a silent open mic; a false "On" would be a false alarm). The state SHALL resolve to On/Off as soon as the mixer confirms the value (via read-back success or an `/xremote` push). This is the mute case of the general unreconciled-state handling (Req 8/Req 15).
...
8. Preset **authoring** (create/edit/delete/name) SHALL be ADMIN-only. Capturing a snapshot SHALL gather the current value of every setting the system can set for all configured channels (full, deterministic snapshot), by querying the mixer.
