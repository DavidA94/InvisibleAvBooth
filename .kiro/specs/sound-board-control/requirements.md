...
| **`/headamp/NN/gain` (float, −12.0…+60.0 dB)**                       | Preamp gain                                                   | The XR18's MIDAS preamps provide a **−12 dB to +60 dB** range (72 dB). Source: `xair-api-python` `HeadAmp.gain` documents "float, from -12.0 to 60.0"; corroborated by the XR18 product specification (MIDAS preamps). **CORRECTION 1 (wire value, verified against real hardware + behringer.world forums):** the OSC WIRE value is a **normalized 0.0–1.0 float** (like the fader), NOT raw dB — `0.0` = −12 dB, `1.0` = +60 dB, mapped LINEARLY. The driver converts dB↔0–1 via `gainDbToFloat`/`gainFloatToDb`. Sending raw dB makes the console clamp to +60 dB and the official app show a different value. **CORRECTION 2 (address, verified against a real XR18):** X Air is NOT X32 here — the address is **2‑digit and 1‑based** (`/headamp/01` = channel 1), NOT X32's 0‑based 3‑digit `/headamp/000`. The XR18 silently ignores the wrong address (no query/set reply), leaving gain stuck at −12 dB and writes ineffective. Confirmed by `xair-api-python` (`/headamp/{index+1 zfill 2}`). See `docs/setup.md` for the per-model debug procedure. |
...
2. THE driver SHALL map interface operations to X Air OSC addresses as documented in "Provenance of Model-Specific Values": fader → `/ch/NN/mix/fader`, mute → `/ch/NN/mix/on` (inverted: `1` = unmuted), gain → `/headamp/NN/gain` (2‑digit, 1‑based; X Air ≠ X32 — see provenance table), name → `/ch/NN/config/name`. THE driver SHALL correctly handle the inverted mute sense so that "muted" in the interface maps to `mix/on = 0`.
