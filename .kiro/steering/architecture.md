...
- **Mixer / Audio Control:** `@mxfriend/osc` (pinned) for OSC-over-UDP control of the Behringer X Air mixer family (fader/mute/gain/presets, `/xremote` subscription, `/meters` metering, read-back reconciliation). Isolated per-channel audio capture for the gain window runs through **PipeWire** + `gstreamer1.0-pipewire` (`pipewiresrc` → `deinterleave`), which owns the mixer's USB device and fans it out to multiple consumers (OBS main-mix + our per-channel capture) without exclusive-open conflicts. `@mxfriend/osc`'s documented fallback is `osc` (osc.js), noted in the driver source.
...
- **Audio Control:** Sound Board widget controlling a Behringer X Air mixer over OSC — faders, mute, per-channel metering, preamp gain with a live gain window (post-preamp envelope over PipeWire capture), and one-tap board presets. Capability-driven per device; degrades gracefully by capability. See `sound-board-control` spec.
...
- **Backend ↔ Preview Clients:** Dedicated `/preview/*` WebSocket endpoints deliver binary media to dashboard widgets. These use the `ws` library (not Socket.io) for minimal-overhead binary transport. Authentication uses the same HttpOnly JWT cookie — the browser sends it automatically on same-origin WebSocket upgrade requests routed through Caddy. `PreviewUpgradeRouter` owns the single `server.on("upgrade")` registration for `/preview/*`, verifies the cookie JWT once, and dispatches by path to a media handler. `VideoPreviewManager` (formerly `PreviewStreamManager`) serves fMP4 video — `/preview/obs` for the OBS NDI output, `/preview/camera/:cameraId` for each camera — with lazy GStreamer spawn, fan-out to subscribers, grace-period teardown, hardware encoder selection, and OBS NDI audio metering. `AudioPreviewManager` serves audio-level previews — `/preview/mixer/:mixerId/channel/:channel` streams the gain-window envelope (its first consumer). The `ndiOutputName` field in OBS device metadata enables OBS preview via NDI (decoupled from RTMP relay — preview works even when not streaming).
- **Backend ↔ Mixer (Mixer HAL):** The `MixerControlInterface` HAL is the single backend abstraction for all mixer communication — callers express intent (set fader/mute/gain, read channel state, activate/capture presets, observe meters, monitor a channel) and the driver (`BehringerXAirDriver`) decides internally whether that is OSC polling, `/xremote` subscription, `/meters` blob decode, or USB capture. OSC is fire-and-forget over UDP; the mixer is authoritative via read-back reconciliation with bounded retry. `MixerService` owns instance lifecycle, state authority, per-mixer metering ref-counting, and connection-preserving hot-reload.
- **Backend ↔ Audio Capture Layer:** `AudioCaptureService` owns the mixer's USB device via PipeWire (`pipewiresrc` → `deinterleave`) and fans isolated per-channel audio out to multiple consumers without exclusive-open conflicts. It selects the USB slot from the admin-configured channel→USB-slot map (not assumed identity), computes a decimated min/max envelope on the backend (not raw PCM), lazily spawns on first subscriber and tears down on last, and is the single owner of capture-pipeline respawn. The gain-window envelope (surfaced to the frontend by `AudioPreviewManager` over `/preview/mixer/*`) is the first consumer; multitrack recording is a designed-for future consumer. **Currently unused:** the gain-window live audio view (the only consumer of this layer) was removed from the shipping UI — see `sound-board-control` spec Appendix A (reference commit `0ea204d`). `AudioCaptureService`, `AudioPreviewManager`, and the `/preview/mixer/*` endpoint are retained (general-purpose, tested) but have no active consumer until multitrack recording or a revived gain window is built.
...
**Exception:** The `/preview/*` WebSocket endpoints are raw binary transport (fMP4 video chunks and mixer envelope frames via the `ws` library) and do not use Socket.io or the event naming convention. They have no named events — the server sends binary `Buffer` frames and the client receives them as `MessageEvent` data. Authentication is cookie-based (same HttpOnly JWT), verified once by `PreviewUpgradeRouter`, which dispatches by path to `VideoPreviewManager` (video) or `AudioPreviewManager` (mixer audio) — not the `SocketGateway`. These are data streams, not command/event channels.
...
| `BUS_MIXER_DEVICE_CHANGED`   | `adminDeviceRoutes` (POST/PUT/DELETE soundboard) | `MixerService`        | Adds, updates, or removes the mixer instance (connection-preserving: only host/port/model reconnects) and re-broadcasts state |
