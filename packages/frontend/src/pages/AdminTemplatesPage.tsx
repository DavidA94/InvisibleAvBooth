...
export function AdminTemplatesPage(): ReactNode {
  ...
  const handleSave = async (): Promise<void> => {
    ...
    try {
      ...
      const res = await fetch(isEdit ? `/api/admin/templates/${formState!.template!.id}` : "/api/admin/templates", {
          ...
          <div key={t.id} data-testid={`${TEST_ID_TEMPLATE_ITEM}-${t.id}`} className="tpl-item">
            ...
            {!isNone(t) && (
              <div className="layout-row gap-tight">
                <button
                  data-testid={`${TEST_ID_TEMPLATE_EDIT_BUTTON}-${t.id}`}
                  className="button-outline button-padding-compact"
                  onClick={() => openForm("edit", category, t)}
                >
                  Edit
                </button>
                <button
                  data-testid={`${TEST_ID_TEMPLATE_DELETE_BUTTON}-${t.id}`}
    ...
    <IonPage data-testid={TEST_ID_ADMIN_TEMPLATES_PAGE}>
      <IonContent className="ion-padding">
        ...
        <Modal
          ...
          onClose={closeForm}
          size="small"
          header={formState?.mode === "edit" ? "Edit Template" : `New ${isDescription ? "Description" : "Title"} Template`}
          footer={formFooter}
        >
          <div className="tpl-form-grid">
            ...
            <div data-testid={TEST_ID_TEMPLATE_FORM_ROLE}>
              <Select<RoleOption>
                ...
                styles={darkSelectStyles<RoleOption>()}
              />
            </div>
          </div>
