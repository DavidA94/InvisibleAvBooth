...
describe("buildInitialState", () => {
  it("returns blank defaults when device is null", () => {
    expect(buildInitialState(null)).toEqual({ label: "", host: "", port: DEFAULT_PORT, password: "", enabled: true });
  });

  it("copies fields from device and clears password when editing", () => {
    expect(buildInitialState(DEVICE)).toEqual({ label: "Main OBS", host: "10.0.0.1", port: "4455", password: "", enabled: true });
  });
...
const BASE: ObsFormState = { label: "L", host: "H", port: "4455", password: "", enabled: true };
