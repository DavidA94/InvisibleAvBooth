...
describe("AddLowerThirdDialog — Title type", () => {
  it("renders title input", () => {
    ...
    expect(screen.getByTestId("lower-third-add-title-input")).toBeInTheDocument();
  });

  it("save is disabled when title is empty", () => {
    render(<AddLowerThirdDialog type="Title" onSave={onSave} onGoLive={onGoLive} onCancel={onCancel} />);
    expect(screen.getByTestId("lower-third-add-save")).toBeDisabled();
  });

  it("calls onSave with correct input when title is filled", () => {
    render(<AddLowerThirdDialog type="Title" onSave={onSave} onGoLive={onGoLive} onCancel={onCancel} />);
    fireEvent.change(screen.getByTestId("lower-third-add-title-input"), { target: { value: "Speaker" } });
...
describe("AddLowerThirdDialog — TitleSubtitle type", () => {
  it("renders title and subtitle inputs", () => {
    ...
    expect(screen.getByTestId("lower-third-add-title-input")).toBeInTheDocument();
    expect(screen.getByTestId("lower-third-add-subtitle-input")).toBeInTheDocument();
  });

  it("save is disabled when only title is filled", () => {
    render(<AddLowerThirdDialog type="TitleSubtitle" onSave={onSave} onGoLive={onGoLive} onCancel={onCancel} />);
    fireEvent.change(screen.getByTestId("lower-third-add-title-input"), { target: { value: "Name" } });
    expect(screen.getByTestId("lower-third-add-save")).toBeDisabled();
  });
  ...
  it("calls onSave with title and subtitle", () => {
    render(<AddLowerThirdDialog type="TitleSubtitle" onSave={onSave} onGoLive={onGoLive} onCancel={onCancel} />);
    fireEvent.change(screen.getByTestId("lower-third-add-title-input"), { target: { value: "Name" } });
    fireEvent.change(screen.getByTestId("lower-third-add-subtitle-input"), { target: { value: "Role" } });
    fireEvent.click(screen.getByTestId("lower-third-add-save"));
...
describe("AddLowerThirdDialog — Scripture type", () => {
  it("renders scripture input instead of title input", () => {
    ...
    expect(screen.queryByTestId("lower-third-add-title-input")).not.toBeInTheDocument();
  });
...
describe("AddLowerThirdDialog — auto-dismiss", () => {
  it("includes autoDismissMs when toggle is enabled", () => {
    render(<AddLowerThirdDialog type="Title" onSave={onSave} onGoLive={onGoLive} onCancel={onCancel} />);
    fireEvent.change(screen.getByTestId("lower-third-add-title-input"), { target: { value: "Test" } });
    // Enable auto-dismiss toggle — the mock renders a checkbox input
    fireEvent.click(screen.getByTestId("lower-third-add-autodismiss-toggle"));
    fireEvent.click(screen.getByTestId("lower-third-add-save"));
