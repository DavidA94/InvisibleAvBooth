...
/* 
describe(`ObsModule — register (EventBus → ${STC_OBS_STATE.split(":")[0]}: broadcasts)`, () => {
  it(`broadcasts ${STC_OBS_STATE} when ${BUS_OBS_STATE_CHANGED} fires`, async () => {
  ...
  it(`broadcasts ${STC_OBS_ERROR} when ${BUS_OBS_ERROR} fires`, async () => {
  ...
  it(`broadcasts ${STC_OBS_ERROR_RESOLVED} when ${BUS_OBS_ERROR_RESOLVED} fires`, async () => {
    const { httpServer, port } = await buildTestServer(new ObsModule(makeMockObsService() as never));
    cleanups.push(() => httpServer.close());
    const client = await connectClient(port);
    cleanups.push(() => client.close());
    ...
    const received = await new Promise<{ errorCode: string }>((resolve) => {
      client.on(STC_OBS_ERROR_RESOLVED, resolve);
  ...
  it(`broadcasts ${STC_DEVICE_CAPABILITIES} when ${BUS_DEVICE_CAPABILITIES_UPDATED} fires`, async () => {
    const { httpServer, port } = await buildTestServer(new ObsModule(makeMockObsService() as never));
    cleanups.push(() => httpServer.close());
    const client = await connectClient(port);
    cleanups.push(() => client.close());
    ...
    const received = await new Promise<{ deviceId: string }>((resolve) => {
      client.on(STC_DEVICE_CAPABILITIES, resolve);
...
describe(`ObsModule — registerSocket (${CTS_OBS_COMMAND.split(":")[0]}: commands)`, () => {
  it(`routes ${CTS_OBS_COMMAND} startStream to obsService.startStream`, async () => {
    const obsService = makeMockObsService();
    const { httpServer, port } = await buildTestServer(new ObsModule(obsService as never));
    cleanups.push(() => httpServer.close());
    const client = await connectClient(port);
  ...
  it(`routes ${CTS_OBS_COMMAND} stopStream to obsService.stopStream`, async () => {
    const obsService = makeMockObsService();
    const { httpServer, port } = await buildTestServer(new ObsModule(obsService as never));
    cleanups.push(() => httpServer.close());
    const client = await connectClient(port);
  ...
  it(`routes ${CTS_OBS_COMMAND} startRecording to obsService.startRecording`, async () => {
    const obsService = makeMockObsService();
    const { httpServer, port } = await buildTestServer(new ObsModule(obsService as never));
    cleanups.push(() => httpServer.close());
    const client = await connectClient(port);
  ...
  it(`routes ${CTS_OBS_COMMAND} stopRecording to obsService.stopRecording`, async () => {
    const obsService = makeMockObsService();
    const { httpServer, port } = await buildTestServer(new ObsModule(obsService as never));
    cleanups.push(() => httpServer.close());
    const client = await connectClient(port);
  ...
  it("returns success: false for unknown command type", async () => {
    ...
    const result = await new Promise<{ success: boolean }>((resolve) => {
      client.emit(CTS_OBS_COMMAND, { type: "unknown" }, resolve);
    });
    expect(result.success).toBe(false);
  });

  it(`routes ${CTS_OBS_RECONNECT} to obsService.reconnect`, async () => {
    const obsService = makeMockObsService();
    const { httpServer, port } = await buildTestServer(new ObsModule(obsService as never));
    cleanups.push(() => httpServer.close());
    const client = await connectClient(port);
...
describe("ObsModule — emitInitialState", () => {
  it(`emits ${STC_OBS_STATE} on connect with current OBS state`, async () => {
    const obsService = makeMockObsService();
    const { httpServer, port } = await buildTestServer(new ObsModule(obsService as never));
    cleanups.push(() => httpServer.close());
