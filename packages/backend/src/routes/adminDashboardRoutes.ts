...
function parseDashboard(row: DashboardRow) {
...
export function createAdminDashboardRouter(db: Database, authService: AuthService): Router {
  ...
  router.post("/", auth, adminOnly, (req: Request, res: Response): void => {
    const { name, description = "", allowedRoles = [] } = req.body as {
      name?: string;
      description?: string;
      allowedRoles?: string[];
    };
    ...
    db.prepare("INSERT INTO dashboards (id, name, description, allowedRoles, createdAt) VALUES (?, ?, ?, ?, ?)").run(
      id, name, description, JSON.stringify(allowedRoles), createdAt,
    );
  ...
  router.post("/:id/widgets", auth, adminOnly, (req: Request, res: Response): void => {
    ...
    const { widgetId, title, col, row, colSpan, rowSpan, roleMinimum = "AvVolunteer" } = req.body as {
      widgetId?: string; title?: string; col?: number; row?: number;
      colSpan?: number; rowSpan?: number; roleMinimum?: string;
    };
    if (!widgetId || !title || col == null || row == null || colSpan == null || rowSpan == null) {
      res.status(400).json({ error: "widgetId, title, col, row, colSpan, rowSpan are required" }); return;
    }
    const id = randomBytes(16).toString("hex");
    const createdAt = new Date().toISOString();
    try {
      ...
      res.status(409).json({ error: "widgetId already exists in this dashboard" }); return;
    }
  ...
  router.get("/:id/widgets/:widgetId", auth, adminOnly, (req: Request, res: Response): void => {
    ...
    if (!row || row.dashboardId !== req.params["id"]) { res.status(404).json({ error: "Widget not found" }); return; }
    res.json(row);
  });
  ...
  router.put("/:id/widgets/:widgetId", auth, adminOnly, (req: Request, res: Response): void => {
    ...
    db.prepare("UPDATE widget_configurations SET title=?, col=?, row=?, colSpan=?, rowSpan=?, roleMinimum=? WHERE id=?").run(
      title ?? row.title, col ?? row.col, r ?? row.row, colSpan ?? row.colSpan,
      rowSpan ?? row.rowSpan, roleMinimum ?? row.roleMinimum, row.id,
    );
