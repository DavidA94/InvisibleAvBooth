...
export function createKjvRouter(db: Database, authService: AuthService): Router {
  ...
  router.get("/validate", auth, (req: Request, res: Response): void => {
    ...
    const verseEnd = verseEndRaw != null ? parseInt(verseEndRaw, 10) : undefined;

    const invalid = (reason: ReasonCode) => res.json({ valid: false, reason });

    // Check book exists (bookId 1–66)
    const bookExists = db.prepare("SELECT 1 FROM kjv WHERE BOOKID = ? LIMIT 1").get(bookId);
    if (!bookExists) { invalid("BOOK_NOT_FOUND"); return; }
    ...
    if (!chapterExists) { invalid("CHAPTER_NOT_FOUND"); return; }
    ...
    if (!verseExists) { invalid("VERSE_NOT_FOUND"); return; }
    ...
    if (verseEnd != null) {
      ...
      if (!verseEndExists) { invalid("VERSE_END_NOT_FOUND"); return; }
    }
