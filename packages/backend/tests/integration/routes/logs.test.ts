...
describe("POST /api/logs", () => {
  it("writes entries to logger with source: frontend", async () => {
    ...
    const res = await s.agent.post("/api/logs").set("Cookie", cookie).send([{ level: "info", message: "page loaded", userId: "u1" }]);
  ...
  it("handles multiple entries in one batch", async () => {
    ...
    await s.agent.post("/api/logs").set("Cookie", cookie).send([
